...
// Sample borrowed assets data
const borrowedAssets = [
    {
        id: 1,
        serialNumber: 'ASSET-2023-001',
        assetName: 'Dell Optiplex 3070',
        borrower: 'John Doe',
        ...
        status: 'borrowed',
        condition: 'good',
        notes: 'Regular use for programming class'
    },
    {
        id: 2,
        serialNumber: 'ASSET-2023-002',
        assetName: 'Projector Epson EB-980',
        borrower: 'Jane Smith',
        department: 'School of Education',
        borrowedDate: '2023-09-05',
        dueDate: '2023-09-12',
        status: 'overdue',
        condition: 'excellent',
    ...
    {
        id: 3,
        serialNumber: 'ASSET-2023-003',
        assetName: 'Interactive Whiteboard',
        borrower: 'Mike Johnson',
        department: 'School of Business',
        borrowedDate: '2023-09-10',
        dueDate: '2023-09-17',
        status: 'borrowed',
        condition: 'good',
        notes: 'Business presentation setup'
    },
    {
        id: 4,
        serialNumber: 'ASSET-2023-004',
        assetName: 'HP LaserJet Printer',
        borrower: 'Sarah Wilson',
        department: 'Computer Lab room 22',
        borrowedDate: '2023-08-28',
        dueDate: '2023-09-04',
        status: 'returned',
        condition: 'excellent',
...
document.addEventListener('DOMContentLoaded', function() {
    initializeBorrowedAssets();
    setupEventListeners();
    loadBorrowedAssetsTable();

    // Set today's date default for return modal date picker
    const returnDateEl = document.getElementById('returnDate');
    if (returnDateEl) returnDateEl.valueAsDate = new Date();
});

function initializeBorrowedAssets() {
    console.log('Initializing borrowed assets dashboard...');
    // Check for overdue assets and highlight them
    checkOverdueAssets();
...
function loadBorrowedAssetsTable() {
    ...
    console.log('Borrowed assets loaded successfully');
}

function createAssetRow(asset) {
    const row = document.createElement('tr');
    row.dataset.assetId = asset.id;
    row.dataset.serialNumber = asset.serialNumber;
    row.dataset.department = asset.department.toLowerCase();
    ...
    row.innerHTML = `
        ...
        <td>${formatDate(asset.borrowedDate)}</td>
...
function getActionButtons(asset) {
    if (asset.status === 'returned') {
        return `<button class="action-btn view" onclick="viewAsset('${asset.serialNumber}')">View</button>`;
    } else if (asset.status === 'overdue') {
        return `
            <button class="action-btn view" onclick="viewAsset('${asset.serialNumber}')">View</button>
            <button class="action-btn return" onclick="openReturnModal('${asset.serialNumber}', '${asset.assetName}')">Return</button>
        `;
    } else {
        return `
            <button class="action-btn view" onclick="viewAsset('${asset.serialNumber}')">View</button>
            <button class="action-btn return" onclick="openReturnModal('${asset.serialNumber}', '${asset.assetName}')">Return</button>
            <button class="action-btn extend" onclick="openExtendModal('${asset.serialNumber}', '${asset.assetName}')">Extend</button>
        `;
    }
}
